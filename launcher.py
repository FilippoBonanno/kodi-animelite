...
def getSource():
    # Lite: menu radice locale, nessuna fetch da server esterno mandrakodi.
    fanart = "https://www.stadiotardini.it/wp-content/uploads/2016/12/mandrakata.jpg"
    strSource = json.dumps({
        "SetViewMode": "500",
        "items": [
            {
                "title": "[COLOR cyan]AnimeUnity (ricerca)[/COLOR]",
                "myresolve": "animeunity@@_search",
                "thumbnail": "https://www.animeunity.so/images/logo.png",
                "fanart": fanart,
                "info": "Cerca un anime su AnimeUnity"
            },
            {
                "title": "[COLOR cyan]AnimeWorld (ricerca)[/COLOR]",
                "myresolve": "animeworld@@_search",
                "thumbnail": "https://www.animeworld.ac/favicon.ico",
                "fanart": fanart,
                "info": "Cerca un anime su AnimeWorld"
            },
            {
                "title": "[COLOR grey]Cambia dominio AnimeUnity/AnimeWorld[/COLOR]",
                "myresolve": "animedomains@@_edit",
                "thumbnail": fanart,
                "fanart": fanart,
                "info": "Aggiorna i domini se i siti sono cambiati"
            }
        ]
    })
    jsonToItems(strSource)

def _injectAnimeUnityMenu(strSource):
    # Voce locale aggiunta SOLO dentro la categoria ANIME & CARTOON (rilevata
    # per contenuto: "ANIME WORLD"/"FILM ANIMAZIONE" sono le voci server-side
    # gia' presenti li', non per URL, che e' remoto e non sotto il nostro
    # controllo). La fonte "ANIME WORLD" e' segnata offline e il resolver
    # 'animeunity' non esisteva nel codice: qui resta raggiungibile anche se
    # il menu server non viene mai corretto.
    import json
    fanart = "https://www.stadiotardini.it/wp-content/uploads/2016/12/mandrakata.jpg"
    local_items = [
        {
            "title": "[COLOR cyan]AnimeUnity (ricerca)[/COLOR]",
            "name": "AnimeUnity (ricerca)",
            "myresolve": "animeunity@@_search",
            "thumbnail": "https://www.animeunity.so/images/logo.png",
            "fanart": fanart,
            "info": "Cerca un anime su AnimeUnity"
        ...
        {
            "title": "[COLOR grey]Cambia dominio AnimeUnity/AnimeWorld[/COLOR]",
            "name": "Cambia dominio AnimeUnity/AnimeWorld",
            "myresolve": "animedomains@@_edit",
            "thumbnail": "https://www.stadiotardini.it/wp-content/uploads/2016/12/mandrakata.jpg",
            "fanart": fanart,
            "info": "Aggiorna i domini se i siti sono cambiati"
        }
    ]
    try:
        data = json.loads(strSource)
        if "channels" in data:
            # ANIME & CARTOON e' un livello "channels" (ogni voce e' sempre
            # una folder che apre un sotto-blocco "items" proprio, vedi
            # jsonToChannels/channelToItems): serve un channel proprio, un
            # item dentro "items" qui non verrebbe mai letto.
            for it in local_items:
                data["channels"].append({
                    ...
                    "fanart": it["fanart"],
                    "info": it["info"],
                    "items": [{k: v for k, v in it.items() if k != "name"}]
                })
        else:
            data.setdefault("items", []).extend(
                [{k: v for k, v in it.items() if k != "name"} for it in local_items]
            )
...
# Sotto questa durata (secondi) il contenuto e' considerato live/instabile
# (Ace Stream, dlhd/daddy, canali): niente resume/watched. Gate a runtime,
# decisione presa perche' jsonToItems non ha modo di sapere a priori se un
# item e' VOD o live (nessun campo dedicato nello schema JSON sorgente).
MIN_VOD_TOTAL_SECONDS = 60


class _ResumePlayer(xbmc.Player):
    ...
    def _persist(self):
        ...
        self._saved = True
        self._refresh()
        if self.cur_total <= MIN_VOD_TOTAL_SECONDS:
            # live/stream instabile: nessun resume/watched (vedi MIN_VOD_TOTAL_SECONDS)
            return
        threshold = 90
        try:
            threshold = int(float(xbmcaddon.Addon(id=addon_id).getSetting("watchedThreshold")))
            threshold = max(50, min(100, threshold))
        except Exception:
            pass
        watched_store.save_progress(self.item_key, self.cur_time, self.cur_total, threshold)

    def onAVStarted(self):
        # gli stream live spesso riportano 0 all'avvio: qualche breve retry
        for _ in range(5):
            try:
...
def _monitorPlayback(item_key):
    player = _ResumePlayer(item_key)
    monitor = xbmc.Monitor()
    # setResolvedUrl() e' asincrono: subito dopo, isPlaying() e' spesso ancora
    # False (buffering/setup codec in corso). Aspetta l'avvio reale prima di
    # trattare "non in riproduzione" come "riproduzione terminata", altrimenti
    # si esce subito con durata 0 e non si salva mai nulla.
    started = False
    for _ in range(30):
    ...
    if not started:
        return
    # Poll periodico finche' il contenuto e' in riproduzione: nessun thread
    # dedicato, il loop blocca solo per la durata della riproduzione stessa
    # (pattern standard Kodi: xbmc.Monitor + waitForAbort).
...
def get_item_identifier(item):
    """Campo usato per calcolare l'item_key stabile (watched_store): link e'
    il campo che gia' identifica univocamente il contenuto in quasi tutti i
    branch di jsonToItems; i fallback coprono gli item che non popolano link."""
    for fld in ('link', 'regexPage', 'myresolve', 'acelocal', 'acehls'):
...
def jsonToItems(strJson):
    ...
    try:
        ...
        # "ANIME & CARTOON" e' costruito interamente lato server (a volte via
        # rete, a volte da un blocco "channels" gia' scaricato in memoria):
        # qui e' l'unico punto per cui passa OGNI possibile percorso, quindi
        # e' l'unico posto affidabile per riconoscerlo per contenuto e
        # agganciare la voce locale AnimeUnity, indipendentemente da come ci
        # si e' arrivati.
        if "ANIME WORLD" in strJson and "FILM ANIMAZIONE" in strJson:
            strJson = _injectAnimeUnityMenu(strJson)
    ...
    try:
        for item in dataJson["items"]:
            ...
            item_key = watched_store.make_key(get_item_identifier(item))
            watched_entry = watched_store.load_state(item_key) if item_key else None
            videoInfo = {'title': titolo,'genre': genre,'plot': info,'mediatype': 'movie','credits': 'ElSupremo'}
            if watched_entry:
                videoInfo['playcount'] = int(watched_entry.get('playcount', 0))
            ...
            if extLink == True:
                ...
                if item_key:
                    url = get_url(action='regex', url=link, exp=regExp, key=item_key)
                else:
                    url = get_url(action='regex', url=link, exp=regExp)
            elif is_myresolve == True:
                if item_key:
                    url = get_url(action='myresolve', url=link, parIn=resolverPar, key=item_key)
                else:
                ...
                if 'apk' in item:
                    ...
                    if not link.startswith("plugin://plugin"):
                        if item_key:
                            url = get_url(action='play', url=link, key=item_key)
                        else:
...
def callReolver(metodo, parametro, item_key=None):
    global viewmode
    import myResolver
    ...
    if metodo=="daddy" and "dlhd.so" in parametro:
        ...
        if isinstance(retVal, list):
            ...
            list_item.setProperty('IsPlayable', 'true')
            if item_key:
                url = get_url(action='play', url=retVal, key=item_key)
            else:
    ...
    try:
        if not sys.argv[2]:
            ...
            if action == 'getExtData':
                ...
                play_video(url, params.get('key'))
            elif action == 'myresolve':
                parIn =  params['parIn']
                logga("MyResolver: "+url+" - "+parIn)
                callReolver(url, parIn, params.get('key'))
