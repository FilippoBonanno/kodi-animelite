...
def _animeDomain(settingId, default):
    # Domini configurabili da Impostazioni Addon: se il sito cambia dominio
    # (pratica comune per questi siti) l'utente lo aggiorna da qui invece
    # di aspettare una nuova build dell'addon.
    try:
        val = xbmcaddon.Addon(id=addon_id).getSetting(settingId)
        val = (val or "").strip().rstrip("/")
        return val if val.startswith("http") else default
    except Exception:
        return default

def _promptAnimeDomain(settingId, label, default):
    dialog = xbmcgui.Dialog()
    current = _animeDomain(settingId, default)
    newVal = dialog.input("Dominio "+label, defaultt=current)
    if newVal:
        newVal = newVal.strip().rstrip("/")
        if newVal.startswith("http"):
            xbmcaddon.Addon(id=addon_id).setSetting(settingId, newVal)

def animeDomains(parIn):
    # Voce menu "Cambia dominio Anime": due dialog in sequenza, uno per sito,
    # pre-compilati col valore attuale. Nessuna sotto-lista da mostrare.
    _promptAnimeDomain("animeunityDomain", "AnimeUnity", "https://www.animeunity.so")
    _promptAnimeDomain("animeworldDomain", "AnimeWorld", "https://www.animeworld.ac")
    xbmcgui.Dialog().notification("Mandrakodi", "Domini aggiornati", xbmcgui.NOTIFICATION_INFO, 3000)
    return None

def _auSessionCachePath():
    profile = xbmcvfs.translatePath(xbmcaddon.Addon(id=addon_id).getAddonInfo('profile'))
    if not xbmcvfs.exists(profile):
        xbmcvfs.mkdirs(profile)
    return profile + "au_session.json"

def _auSession(base):
    # Ogni chiamata al plugin e' un processo Kodi nuovo: senza persistenza,
    # ogni ricerca apriva una sessione/cookie mai visti prima dallo stesso IP,
    # in rapida successione durante i test -> pattern tipico bloccato dagli
    # anti-bot (risposta 200 con dati fittizi invece di errore esplicito).
    # Riusando cookie/CSRF gia' ottenuti (validi ~10 minuti) si evita di
    # aprire una "nuova sessione" ad ogni ricerca.
    import json, time
    ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36"
    s = requests.Session()
    path = _auSessionCachePath()
    cached = None
    try:
    ...
    now = time.time()
    if cached and (now - cached.get("ts", 0)) < 600 and cached.get("csrf"):
        for name, value in cached.get("cookies", {}).items():
            s.cookies.set(name, value)
        return s, cached.get("ua", ua), cached.get("csrf")

    home = s.get(base+"/", headers={"User-Agent": ua})
    csrf = preg_match(home.text, r'csrf-token" content="(.*?)"')
    try:
        cookies_dict = requests.utils.dict_from_cookiejar(s.cookies)
        f = xbmcvfs.File(path, 'w')
        f.write(json.dumps({"ts": now, "ua": ua, "csrf": csrf, "cookies": cookies_dict}))
        f.close()
    except Exception:
        pass
    return s, ua, csrf

def _auHeaders(base, ua, csrf=None, ajax=True):
    # Origin/Referer/Sec-Fetch-* sono quello che il WAF usa per distinguere
    # una chiamata AJAX "legittima" (partita dalla pagina del sito) da una
    # richiesta diretta all'endpoint: senza, /livesearch risponde 200 con un
    # risultato fisso invece dei dati filtrati (nessun errore esplicito).
    h = {
        "User-Agent": ua,
        "Origin": base,
...
def animeunity(parIn):
    ...
    # (nessuno stato Python persiste tra un livello di menu e il successivo):
    #   ricerca titolo -> "a:<anime_id>" lista episodi -> "e:<episode_id>" stream finale.
    import json
    logga("ANIMEUNITY IN: "+str(parIn))
    video_urls = []
    base = _animeDomain("animeunityDomain", "https://www.animeunity.so")
    try:
        if parIn == "_search":
            dialog = xbmcgui.Dialog()
            title = dialog.input("Cerca anime su AnimeUnity")
            if not title:
                return None
            parIn = title
        ...
        if parIn.startswith("a:"):
            ...
            for i, ep in enumerate(episodes):
                ...
                jsonText += '"info":"AnimeUnity"}'
            if not episodes:
                jsonText += '{"title":"[COLOR red]NESSUN EPISODIO[/COLOR]","link":"ignore",'
                jsonText += '"thumbnail":"https://www.animeunity.so/images/logo.png",'
                jsonText += '"fanart":"https://www.stadiotardini.it/wp-content/uploads/2016/12/mandrakata.jpg","info":"No episodes"}'
            jsonText += ']}'
            video_urls.append((jsonText, "PLAY VIDEO", "No info", "noThumb", "json"))
            return video_urls

        if parIn.startswith("e:"):
            ep_id = parIn[2:]
            r = s.get(base+"/embed-url/"+ep_id, headers=_auHeaders(base, ua, csrf, ajax=False))
            embedUrl = r.text.strip()
            if not embedUrl.startswith("http"):
                video_urls.append(("ignore", "[COLOR red]EMBED NON TROVATO[/COLOR]"))
                return video_urls
            rEmb = s.get(embedUrl, headers={"User-Agent": ua, "Referer": base+"/"})
        ...
        # Il server, sotto rate-limit/anti-bot, risponde 200 con una lista
        # fissa "popolare" che ignora "title" invece di un errore esplicito.
        # Se nessun risultato contiene una sola parola della query, e' quasi
        # certamente questo fallback silenzioso: lo segnaliamo invece di
        # mostrare una lista che sembra valida ma non c'entra nulla.
        query_words = [w for w in parIn.lower().split() if len(w) >= 3]
        def _auRecMatches(rec):
            if not query_words:
                return True
            combined = " ".join(str(rec.get(k) or "") for k in ("title", "title_it", "title_eng", "slug")).lower()
            return any(w in combined for w in query_words)
        rate_limited = bool(records) and not any(_auRecMatches(rec) for rec in records)
        if rate_limited:
            # la sessione riusata e' probabilmente segnata dall'anti-bot:
            # cancellala cosi' il prossimo tentativo riparte da una pulita
            # invece di ripetere la stessa richiesta con lo stesso cookie.
            try:
                path = _auSessionCachePath()
                if xbmcvfs.exists(path):
                    xbmcvfs.delete(path)
        ...
        jsonText = '{"SetViewMode":"51","items":['
        if not records:
            jsonText += '{"title":"[COLOR red]NESSUN RISULTATO[/COLOR]","link":"ignore",'
            jsonText += '"thumbnail":"https://www.animeunity.so/images/logo.png",'
            jsonText += '"fanart":"https://www.stadiotardini.it/wp-content/uploads/2016/12/mandrakata.jpg","info":"No result"}'
        elif rate_limited:
            jsonText += '{"title":"[COLOR red]AnimeUnity non ha filtrato la ricerca (troppe richieste, riprova tra qualche minuto)[/COLOR]","link":"ignore",'
            jsonText += '"thumbnail":"https://www.animeunity.so/images/logo.png",'
            jsonText += '"fanart":"https://www.stadiotardini.it/wp-content/uploads/2016/12/mandrakata.jpg","info":"Rate limit"}'
        else:
            for i, rec in enumerate(records):
                if i > 0:
                    jsonText += ','
                aid = rec.get("id")
                tit = (rec.get("title_it") or rec.get("title_eng") or rec.get("title") or rec.get("slug") or "AnimeUnity")
                tit = tit.replace('"', "'")
                img = rec.get("imageurl", "")
                plot = (rec.get("plot") or "").replace('"', "'").replace("\n", " ").replace("\r", " ")
                jsonText += '{"title":"[COLOR lime]'+tit+'[/COLOR]",'
                jsonText += '"myresolve":"animeunity@@a:'+str(aid)+'",'
                jsonText += '"thumbnail":"'+img+'",'
                jsonText += '"fanart":"https://www.stadiotardini.it/wp-content/uploads/2016/12/mandrakata.jpg",'
                jsonText += '"info":"'+plot+'"}'
        jsonText += ']}'
        video_urls.append((jsonText, "PLAY VIDEO", "No info", "noThumb", "json"))
        ...
        import traceback
        traceback.print_exc()
        errMsg="ERRORE ANIMEUNITY: {0}".format(err)
        dialog = xbmcgui.Dialog()
        dialog.ok("Mandrakodi", errMsg)
...
def animeworld(parIn):
    ...
    # stream finale. Qui la risposta di /api/episode/info include gia' un
    # link mp4 diretto ("grabber"), niente embed intermedio da seguire.
    import json
    logga("ANIMEWORLD IN: "+str(parIn))
    video_urls = []
    base = _animeDomain("animeworldDomain", "https://www.animeworld.ac")
    ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36"
    thumb = "https://www.animeworld.ac/favicon.ico"
    fanart = "https://www.stadiotardini.it/wp-content/uploads/2016/12/mandrakata.jpg"
    try:
        if parIn == "_search":
            dialog = xbmcgui.Dialog()
            title = dialog.input("Cerca anime su AnimeWorld")
            if not title:
                return None
            parIn = title
        ...
        if parIn.startswith("a:"):
            ...
            for i, (epId, tok, num) in enumerate(eps):
                if i > 0:
                    jsonText += ','
                jsonText += '{"title":"[COLOR gold]Episodio '+num+'[/COLOR]",'
                jsonText += '"myresolve":"animeworld@@e:'+tok+'",'
                jsonText += '"thumbnail":"'+thumb+'",'
                jsonText += '"fanart":"'+fanart+'",'
                jsonText += '"info":"AnimeWorld"}'
            if not eps:
                jsonText += '{"title":"[COLOR red]NESSUN EPISODIO[/COLOR]","link":"ignore",'
                jsonText += '"thumbnail":"'+thumb+'","fanart":"'+fanart+'","info":"No episodes"}'
            jsonText += ']}'
            video_urls.append((jsonText, "PLAY VIDEO", "No info", "noThumb", "json"))
            return video_urls
        ...
        if parIn.startswith("e:"):
            token = parIn[2:]
            # il csrf token e' legato alla sessione, non alla pagina: basta
            # la home per ottenerne uno valido, senza dover ricostruire l'URL
            # completo dell'anime (che qui non e' disponibile).
            rHome = s.get(base+"/", headers={"User-Agent": ua})
            csrf = preg_match(rHome.text, r'id="csrf-token" content="(.*?)"')
            h = {"User-Agent": ua, "CSRF-Token": csrf, "X-Requested-With": "XMLHttpRequest",
        ...
        # href e data-jtitle compaiono 2 volte per risultato (poster+nome): dedup per href
        seen = set()
        results = []
        for href, jtitle in re.findall(r'<a href="(/play/[^"]+)"[^>]*data-jtitle="([^"]*)"', r.text):
            if href not in seen:
                seen.add(href)
                results.append((href, jtitle))
        jsonText = '{"SetViewMode":"51","items":['
        if not results:
            jsonText += '{"title":"[COLOR red]NESSUN RISULTATO[/COLOR]","link":"ignore",'
            jsonText += '"thumbnail":"'+thumb+'","fanart":"'+fanart+'","info":"No result"}'
        else:
            for i, (href, jtitle) in enumerate(results):
                if i > 0:
                    jsonText += ','
                slugId = href.replace("/play/", "")
                tit = jtitle.replace('"', "'") or slugId
                jsonText += '{"title":"[COLOR lime]'+tit+'[/COLOR]",'
                jsonText += '"myresolve":"animeworld@@a:'+slugId+'",'
                jsonText += '"thumbnail":"'+thumb+'",'
                jsonText += '"fanart":"'+fanart+'",'
                jsonText += '"info":"AnimeWorld"}'
        jsonText += ']}'
        ...
        import traceback
        traceback.print_exc()
        errMsg="ERRORE ANIMEWORLD: {0}".format(err)
        dialog = xbmcgui.Dialog()
        dialog.ok("Mandrakodi", errMsg)
        video_urls.append(("ignore", "[COLOR red]ERRORE[/COLOR]"))
        return video_urls
...
def run (action, params=None):
    ...
    if action in commands:
        ...
        raise ValueError('Invalid command: {0}!'.format(action))